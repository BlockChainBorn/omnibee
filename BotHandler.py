...
BOT_COMMAND_GPT = 'nulu'
BOT_COMMAND_DRAW = 'ritaofixa'
...
class BotHandler:
    ...
    def bot_start(self):
        ...
        try:
            ...
            application.add_handler(CommandHandler(BOT_COMMAND_GPT, self.bot_command_nulu))
            application.add_handler(CommandHandler(BOT_COMMAND_DRAW, self.bot_command_ritaofixa))
    ...
    async def bot_read_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # No message
            ...
            await context.bot.send_message(chat_id=chat_id,
                                           text=str(self.messages['request_no_message']).replace('\\n', '\n'))

    async def bot_command_ritaofixa(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        /ritaofixa command
        :param update:
        :param context:
        ...
        user = update.message.from_user
        chat_id = update.effective_chat.id
        logging.info('/ritaofixa command from user ' + str(user.full_name) + ' request: ' + ' '.join(context.args))
    ...
    async def bot_command_request(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """
        /request command
        :param update:
        :param context:
        ...
        user = update.message.from_user
        chat_id = update.effective_chat.id
        logging.info('/request command from user ' + str(user.full_name) + ' request: ' + ' '.join(context.args))
        ...
        if len(context.args) > 0:
            ...
            # No text
                ...
                await context.bot.send_message(chat_id=chat_id,
                                               text=str(self.messages['request_no_text']).replace('\\n', '\n'))
        # No text
        else:
            await context.bot.send_message(chat_id=chat_id,
                                           text=str(self.messages['request_no_text']).replace('\\n', '\n'))
